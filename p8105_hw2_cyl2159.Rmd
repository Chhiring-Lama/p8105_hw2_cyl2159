...
The NYC transit dataset initially had `r ncol(nyc_transit_df)` columns and `r nrow(nyc_transit_df)` rows. I recognized that some routes had empty cells, so I replaced the values with `NA`, and introduced snake-case column names. I used `case_match` within `mutate` to convert "YES" and "NO" values to logical variables. The resulting dataframe has `r ncol(nyc_transit_clean_df)` columns and `r nrow(nyc_transit_clean_df)` rows. The data is not yet tidy because values of a same variable (route number) is spread across multiple columns. We will tidy the data further in the later portion of problem 1. 
...
Calculating the number of entrances/ exits without vending, and those that allow entrance among them. 
```{r}
...
nyc_transit_clean_long_df <- nyc_transit_clean_df |> 
  ...
  drop_na(subway_line) 
```
- I cleaned the dataset further by gathering the columns between `route1` and `route11` with `pivot_longer` into two columns `route_number` and `subway_line`. This step introduced columns with NA values for `subway_line` which are redundant rows, so I dropped the rows with missing data for `subway_line`. The resulting dataset has `r ncol(nyc_transit_clean_long_df)` columns containing the following variables: ``r names(nyc_transit_clean_long_df)[1:11]`` and ``r names(nyc_transit_clean_long_df)[12]``. There are `r nrow(nyc_transit_clean_long_df)` observations/rows where each row corresponds to information about a specific subway line served by an entrance at a subway station. 
